...
from core.hcdc import apply_hcdc, HCDCResult
from core.terrain_solver import solve_terrain, TerrainResult
from core.candidate_generator import generate_candidates, CandidateGeometry
from core.counterfactual_height import run_chf_iscl, CHFResult
from core.shadow_test import run_shadow_test, ShadowTestResult
from core.occlusion_test import run_occlusion_test, OcclusionResult
from core.scoring_engine import run_egss, EvidenceScore
from core.self_disproof import run_sdrl, SDRLResult
...
def run_pipeline(
    ...
    log.info("Image: %dx%d | Depth: %dx%d | Scale: RELATIVE",
    ...
    log.info("Phase 3 done | %d buildings | quality=%.2f",
             len(structure_result.buildings), structure_result.segmentation_quality)
    ...
    terrain_result = solve_terrain(corrected_depth, structure_result, input_data.georef, config)
    phase_times["phase10_terrain"] = time.perf_counter() - t10

    # ── Phase 5: Candidate Generation ─────────────────────────
    log.info("--- Phase 5: Candidate Generation ---")
    t5 = time.perf_counter()
    candidates = generate_candidates(structure_result, terrain_result, corrected_depth, config)
    phase_times["phase5_candidates"] = time.perf_counter() - t5
    log.info("Phase 5 done | %d candidates across %d buildings",
             len(candidates), len(structure_result.buildings))

    if not candidates:
        log.warning("No candidates generated — pipeline cannot continue. Returning partial result.")
        # Return minimal result
        return _make_empty_result(input_data, depth_result, structure_result,
                                  hcdc_result, terrain_result, phase_times, t_total)

    # ── Phase 6+7: CHF + ISCL ─────────────────────────────────
    log.info("--- Phase 6+7: CHF + ISCL ---")
    t6 = time.perf_counter()
    chf_result = run_chf_iscl(candidates, corrected_depth, structure_result, terrain_result, config)
    ...
    shadow_result = run_shadow_test(
        image_rgb, corrected_depth, structure_result, candidates, input_data.georef, config
    )
    ...
    occlusion_result = run_occlusion_test(candidates, corrected_depth, structure_result, config)
    ...
    evidence_scores = run_egss(
        candidates, chf_score_lookup, shadow_result,
        occlusion_result, terrain_result, structure_result, config
    )
    ...
    log.info("  Buildings: %d | Candidates: %d | Survived: %d | Rejected: %d",
             len(structure_result.buildings), len(candidates),
...
def _make_empty_result(input_data, depth_result, structure_result,
                        hcdc_result, terrain_result, phase_times, t_total) -> PipelineResult:
    ...
    return PipelineResult(
        ...
        shadow_result=ShadowTestResult(False, False, 0.0, None, None, None, 0.0, {}, 0.0, 0.0),
