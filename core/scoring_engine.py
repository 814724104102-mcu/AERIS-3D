...
def run_egss(
    ...
    log.info("Running EGSS | candidates=%d | rejection_threshold=%.2f",
             len(candidates), rejection_threshold)
    ...
    total_w = (w_depth + w_edge + w_boundary_eff + w_shadow_eff
               + w_occlusion + w_terrain_eff + w_struct)
    if total_w < 1e-6:
        total_w = 1.0

    active_evidence = []
    if w_depth > 0:    active_evidence.append("depth_agreement")
    if w_edge > 0:     active_evidence.append("edge_overlap")
    if w_boundary_eff > 0: active_evidence.append("segmentation_boundary")
    if w_shadow_eff > 0:   active_evidence.append("shadow")
    if w_occlusion > 0:    active_evidence.append("occlusion")
    if w_terrain_eff > 0:  active_evidence.append("terrain")
    if w_struct > 0:       active_evidence.append("structural_plausibility")
    ...
    log.info("EGSS effective weights: %s", {k: f"{v:.3f}" for k, v in final_weights.items()})
    ...
    for cand in candidates:
        ...
        region = next((r for r in structure_result.regions if r.object_id == cand.object_id), None)
        if region is not None:
            ...
            depth_range = max(
                terrain_result.object_height_map.max() - terrain_result.object_height_map.min(),
                1e-6
            )
            # Map depth height to a height-unit-comparable value
            depth_predicted_h = terrain_height_px / depth_range * 60.0
            terrain_score = float(np.exp(-0.01 * (cand.height_value - depth_predicted_h) ** 2))
        else:
            terrain_score = 0.5
        ...
        overall = sum(
            component_scores[k] * final_weights[k]
    ...
    log.info("EGSS done | %d/%d candidates pass threshold=%.2f | %.3fs",
             n_pass, len(results), rejection_threshold, runtime_s)
    return results
