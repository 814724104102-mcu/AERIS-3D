...
def run_pdu(
    ...
    log.info(
        "Running PDU | n_iter=%d | depth_sigma_frac=%.3f", n_iter, depth_sigma_frac
    )
        ...
        for cand in candidates:
            ...
            depth_at_cand = float(
                perturbed[
                    int(
                        np.clip(
                            cand.geometry_parameters.get("centroid", [0, 0])[0],
                            0,
                            perturbed.shape[0] - 1,
                        )
                    ),
                    int(
                        np.clip(
                            cand.geometry_parameters.get("centroid", [0, 0])[1],
                            0,
                            perturbed.shape[1] - 1,
                        )
                    ),
            ...
            if abs(actual_offset) > 1e-5:
                ...
                perturbed_depth_agree = float(
                    np.exp(-2.0 * abs(np.log(max(ratio, 1e-3))))
                )
            else:
            ...
            perturbed_score = float(
                np.clip(
                    ...
                    + depth_w
                    * (
                        perturbed_depth_agree
                        - es.component_scores.get("depth_agreement", 0.3)
                    ),
                    0.0,
                    1.0,
                )
    ...
    log.info(
        "PDU done | %d candidates | n_iter=%d | %.3fs",
        len(uncertainties),
        n_iter,
        runtime_s,
    )
