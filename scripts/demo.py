...
def make_height_fingerprint_chart(chf_result, sdrl_result, output_path: Path) -> None:
    ...
    for col_idx, (obj_id, fp) in enumerate(fingerprint.items()):
        ...
        ax.plot(heights, scores, "-", color="#4ecdc4", linewidth=2.0, alpha=0.7, zorder=2)
        ...
        for h_val, sc in zip(heights, scores):
            ...
            ax.scatter([h_val], [sc], c=color, s=size, marker=marker,
                       zorder=3, edgecolors="white", linewidths=0.5)

        # Rejection threshold line
        reject_thresh = 0.35
        ax.axhline(y=reject_thresh, color="#f39c12", linestyle="--",
                   linewidth=1.5, alpha=0.8, label=f"Rejection threshold ({reject_thresh})")
...
def make_candidate_table_image(sdrl_result, evidence_scores, output_path: Path) -> None:
    ...
    survivors_set = {sdrl_result.survivors[first_obj_id].candidate_id} if first_obj_id in sdrl_result.survivors else set()
    rejected_ids = {es.candidate_id for es in sdrl_result.rejected.get(first_obj_id, [])}
    ...
    for es in sorted(obj_scores, key=lambda x: x.overall_score, reverse=True):
        if es.candidate_id in survivors_set:
            ...
            status = "✗ REJECTED"
            color = "#e74c3c"
        rows.append((es.candidate_id, f"{es.height_value:.1f}", f"{es.overall_score:.3f}", status, color))
    ...
    # Title
    ax.text(0.5, 0.97, "AERIS VERIFICATION", transform=ax.transAxes,
            fontsize=16, fontweight="bold", color="#4ecdc4",
            ha="center", va="top", fontfamily="monospace")
    ax.text(0.5, 0.91, f"Object: {first_obj_id}  |  Height Units: {unit}",
    ...
    for col_x_pos, header in zip(col_x, headers):
        ax.text(col_x_pos, header_y, header, transform=ax.transAxes,
                fontsize=11, color="#f39c12", fontweight="bold",
                va="top", fontfamily="monospace")

    # Separator
    ax.plot([0.02, 0.98], [0.80, 0.80], color="#4ecdc4", linewidth=1.0,
    ...
    for i, (cid, h_str, sc_str, stat, col) in enumerate(rows):
        y_pos = 0.78 - i * row_h
        ax.text(col_x[0], y_pos, cid, transform=ax.transAxes,
                fontsize=10, color="white", va="top", fontfamily="monospace")
        ax.text(col_x[1], y_pos, h_str, transform=ax.transAxes,
                fontsize=10, color="white", va="top", fontfamily="monospace")
        ax.text(col_x[2], y_pos, sc_str, transform=ax.transAxes,
                fontsize=10, color="white", va="top", fontfamily="monospace")
        ax.text(col_x[3], y_pos, stat, transform=ax.transAxes,
                fontsize=10, color=col, va="top", fontweight="bold",
                fontfamily="monospace")

    # Footer: best-supported
    if survivor_es:
        ax.plot([0.02, 0.98], [0.18, 0.18], color="#4ecdc4", linewidth=1.0,
                transform=ax.transAxes, clip_on=False)
        ax.text(0.05, 0.14, f"BEST-SUPPORTED HEIGHT:  {survivor_es.height_value:.1f}  ({unit})",
                transform=ax.transAxes, fontsize=12, color="#2ecc71",
                fontweight="bold", va="top", fontfamily="monospace")
        ax.text(0.05, 0.07, f"EGSS score: {survivor_es.overall_score:.4f}  |  "
                f"Scale: {unit}  |  "
...
def run_demo(args: argparse.Namespace) -> int:
    ...
    hw = get_hardware()
    log.info("Hardware: device=%s (%s) CPUs=%d", hw.device, hw.device_name, hw.cpu_count)
    ...
    log.info("Gate 2 PASSED | model=%s device=%s cache=%s runtime=%.2fs",
    ...
    # ── Gate 3: Structures ────────────────────────────────────
    if sr.num_regions == 0:
        log.warning("Gate 3: No structures detected — check segmentation or input image.")
    else:
        log.info("Gate 3 PASSED | %d regions (%d buildings)", sr.num_regions, len(sr.buildings))
    ...
    if corr_max > corr_min:
        ...
        corr_norm = corr_nd
    save_image(colorize_depth(corr_norm.astype(np.float32)), output_dir / "depth_corrected.png")
    ...
    if pr.n_survived == 0:
        ...
        log.warning("Gate 6: No candidates were rejected — increase candidate count or check scoring.")
        gate6_pass = pr.n_survived > 0
    else:
        log.info("Gate 6 PASSED | %d survived | %d rejected", pr.n_survived, pr.n_rejected)
        gate6_pass = True

    # ── Produce visualizations ─────────────────────────────────
    make_height_fingerprint_chart(pr.chf_result, sdrl, output_dir / "height_fingerprint.png")
    make_candidate_table_image(sdrl, pr.evidence_scores, output_dir / "verification_table.png")
    ...
    sdrl_rejected_ids = {
        es.candidate_id
        for lst in sdrl.rejected.values()
        for es in lst
    }
    ...
    for cand in pr.candidates:
        ...
        status = (
            "SURVIVED" if cand.candidate_id in sdrl_survivors_ids
        ...
        row = {
            ...
            "component_scores": {k: round(v, 4) for k, v in es.component_scores.items()} if es else {},
        }
    ...
    results = {
        ...
        "gates_passed": (
            ["Gate1_Input", "Gate2_Depth", "Gate3_Structures",
             "Gate4_Candidates", "Gate5_Scores"]
    ...
    print(f"  Georef:   {'YES — ' + (input_data.georef.crs if input_data.georef else '') if input_data.georef else 'NO'}")
    ...
    print(f"  Candidates: {pr.n_candidates} generated | {pr.n_survived} survived | {pr.n_rejected} rejected")
    print()
    ...
    if first_obj:
        ...
        for es in obj_scores:
            status = "✓ SURVIVED" if es.candidate_id in sdrl_survivors_ids else "✗ REJECTED"
            print(f"  {es.candidate_id:<12} {es.height_value:>7.1f}  {es.overall_score:>8.4f}  {status}")
    print()
    for obj_id, survivor in sdrl.survivors.items():
        print(f"  BEST-SUPPORTED [{obj_id}]: height={survivor.height_value:.1f} "
              f"({survivor.height_unit}) score={survivor.overall_score:.4f}")
        print(f"  ⚠  RELATIVE ESTIMATE — not ground truth")
    print()
    print(f"  Shadow: available={pr.shadow_result.shadow_available} "
    ...
    print()
    print("  Outputs:")
    for name in ["input_preview.png", "depth.png", "structures.png",
                 "depth_corrected.png", "height_fingerprint.png",
                 "verification_table.png", "candidates.json", "results.json"]:
        print(f"    - {name}")
    print("=" * 65)
    all_gates = ["Gate1_Input", "Gate2_Depth", "Gate3_Structures",
                 "Gate4_Candidates", "Gate5_Scores"]
    if gate6_pass:
